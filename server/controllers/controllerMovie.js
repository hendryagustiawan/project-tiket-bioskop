...
class ControllerMovie {
  ...
  static async addMovie(req, res, next) {
    const { title, imgUrl, genre, producer, director, linkYT, writer, synopsis, cast, price, duration, ProductionId } = req.body;
    const UserId = req.userData.id;

    try {
      const newData = await Movie.create({ title, imgUrl, genre, linkYT, producer, director, writer, synopsis, cast, price, duration, UserId, ProductionId });
